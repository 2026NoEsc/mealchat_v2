...
| 컨트롤 | 필요한 것 |
|---|---|
| `지도에서 선택` 칩 | 지도에서 좌표를 찍는 화면. Tmap→카카오 전환과 함께 정하는 편이 낫다 |
| 밥약 메이트 선택 헤더의 돋보기 | 친구 목록 검색·필터 |
| 밥약 메이트 선택 헤더의 `+` | [FriendsScreen](../src/screens/profile/FriendsScreen.tsx) 로 보내면 된다 |
