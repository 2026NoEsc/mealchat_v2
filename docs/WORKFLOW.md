...
Figma 안에서도 요일 표기가 서로 어긋나 있어서 **`8/15 = 금요일`** 로 통일했다.
따라서 `8/1 = 금요일` 이고, [ScheduleHomeScreen](../src/screens/schedule/ScheduleHomeScreen.tsx) 의
`FIRST_COLUMN = 5` 가 이 기준을 담고 있다. 요일을 새로 적을 일이 있으면 여기서 파생시킬 것.

| 날짜 | 요일 |
|---|---|
| 8/1, 8/8, 8/15, 8/22, 8/29 | 금 |
| 8/12 | 화 |
| 8/13 | 수 |
| 8/21 | 목 |

> 실제 2026년 8월 15일은 토요일이지만, Figma 원본 표기를 따랐다.
