...
4. **프로필 수정 `309:1086` / 출발지 설정 `256:2333`** — 프로필 하위 중 남은 두 개.
   (정보 공개 범위 `256:2494` 는 구현 완료)
...
| 위치 | 버튼 | 필요한 것 |
|---|---|---|
| 채팅방 입력바 | 스마일 아이콘 | 이모티콘 패널 `555:416` — 스티커 8종 에셋을 새로 받아야 한다 |
| 회원가입·프로필 수정 | 은행 칩 | 은행 드롭다운 `549:3366` |
| 로그인 | `아이디 비밀번호 찾기` | Figma 에도 화면 없음 |
| 방 상세정보 | `변경` (약속 장소) | 출발지 설정 `256:2333` 재사용 예정 |
| 프로필 | `사는 곳 설정` | 출발지 설정 `256:2333` — 지금은 정보 공개 범위로 임시 연결 |
